...
rep200Data_WGS_RNA_HiSeq_Fungi_Decontam <- rep200Data_WGS_RNA_HiSeq_Fungi[,!(colnames(rep200Data_WGS_RNA_HiSeq_Fungi) %in% contaminants)]
dim(rep200Data_WGS_RNA_HiSeq_Fungi_Decontam) # 15088   248

sum(rowSums(rep200Data_WGS_RNA_HiSeq_Fungi_Decontam) == 0) # 513
emptySamplesAfterDecontam <- names(which(rowSums(rep200Data_WGS_RNA_HiSeq_Fungi_Decontam) == 0))
samples2RemoveWithDecontam <- c(emptySamplesAfterDecontam, missingSeqCenter, na.omit(smallHopkins))

rep200Data_WGS_RNA_HiSeq_Fungi_Decontam_Nonzero <- rep200Data_WGS_RNA_HiSeq_Fungi_Decontam[!(rownames(rep200Data_WGS_RNA_HiSeq_Fungi_Decontam) %in% samples2RemoveWithDecontam),]
metaQiitaWGS_RNA_HiSeq_Filt_Nonzero <- droplevels(metaQiitaWGS_RNA_HiSeq_Filt[!(rownames(metaQiitaWGS_RNA_HiSeq_Filt) %in% samples2RemoveWithDecontam),])

dim(rep200Data_WGS_RNA_HiSeq_Fungi_Decontam_Nonzero) # 14546   248
dim(metaQiitaWGS_RNA_HiSeq_Filt_Nonzero) # 14546    41

## Convert strings to factors
metaQiitaWGS_RNA_HiSeq_Filt_Nonzero$sample_type <- factor(metaQiitaWGS_RNA_HiSeq_Filt_Nonzero$sample_type)
metaQiitaWGS_RNA_HiSeq_Filt_Nonzero$disease_type <- factor(metaQiitaWGS_RNA_HiSeq_Filt_Nonzero$disease_type)
metaQiitaWGS_RNA_HiSeq_Filt_Nonzero$experimental_strategy <- factor(metaQiitaWGS_RNA_HiSeq_Filt_Nonzero$experimental_strategy)
metaQiitaWGS_RNA_HiSeq_Filt_Nonzero$data_submitting_center_label <- factor(metaQiitaWGS_RNA_HiSeq_Filt_Nonzero$data_submitting_center_label)
metaQiitaCombined_Nonzero <- metaQiitaWGS_RNA_HiSeq_Filt_Nonzero

save(rep200Data_WGS_RNA_HiSeq_Fungi_Decontam_Nonzero,
     metaQiitaCombined_Nonzero,
     file = "Interim_data/fungi_data_WGS_RNA_HiSeq_Decontam_13Sep21.RData")

#------------------------------------------------------------------#
# WGS vs. RNA-Seq norm read counts
#------------------------------------------------------------------#

metaQiitaCombined_Nonzero_DNAonly <- metaQiitaCombined_Nonzero %>% filter(experimental_strategy == "WGS") %>% droplevels()
metaQiitaCombined_Nonzero_RNAonly <- metaQiitaCombined_Nonzero %>% filter(experimental_strategy == "RNA-Seq") %>% droplevels()

wgsData <- rep200Data_WGS_RNA_HiSeq_Fungi_Decontam_Nonzero[rownames(metaQiitaCombined_Nonzero_DNAonly),]
rnaData <- rep200Data_WGS_RNA_HiSeq_Fungi_Decontam_Nonzero[rownames(metaQiitaCombined_Nonzero_RNAonly),]
...
combinedSampleCounts %>%
  ...
  stat_n_text() + 
  ggsave(filename = "Figures/Supplementary_Figures/tcga_read_count_wgs_vs_rna.jpeg",
         dpi = "retina",
         height = 5,
         width = 3,
         units = "in")
...
qcMetadata <- metaQiitaCombined_Nonzero 
qcData <- rep200Data_WGS_RNA_HiSeq_Fungi_Decontam_Nonzero 
...
snmDataObjOnly <- snm(raw.dat = vdge_data$E, 
                      ...
                      verbose = TRUE,
                      diagnose = TRUE)
snmDataOGUFungi <- t(snmDataObjOnly$norm.dat)

save(snmDataOGUFungi,
     vdge_dataE,
     rep200Data_WGS_RNA_HiSeq_Fungi_Decontam_Nonzero,
     metaQiitaCombined_Nonzero,
     file = "Interim_data/snmDataFungi_13Sep21.RData")
...
voomPca_SeqCenter <- pcaPlotting(pcaObject = prcomp(vdge_dataE),
                                pcChoices = c(1,2),
                                dataLabels = qcMetadata$data_submitting_center_label,
                                factorString = "Sequencing Center",
                                titleString = "PCA w/o Batch Correction") +
  ggsave(filename = "Figures/Supplementary_Figures/tcga_pca_voom_only_seq_center.jpeg",
              dpi = "retina", width = 6, height = 6, units = "in")

voomPca_ExpStrategy <- pcaPlotting(pcaObject = prcomp(vdge_dataE),
                    pcChoices = c(1,2),
                    dataLabels = qcMetadata$experimental_strategy,
                    factorString = "Experimental Strategy",
                    titleString = "PCA w/o Batch Correction") +
  ggsave(filename = "Figures/Supplementary_Figures/tcga_pca_voom_only_exp_strategy.jpeg",
         dpi = "retina", width = 6, height = 6, units = "in")

vsnmPca_SeqCenter <- pcaPlotting(pcaObject = prcomp(snmDataOGUFungi),
                       pcChoices = c(1,2),
                       dataLabels = qcMetadata$data_submitting_center_label,
                       factorString = "Sequencing Center",
                       titleString = "PCA w/ Voom-SNM Correction") +
  ggsave(filename = "Figures/Supplementary_Figures/tcga_pca_vsnm_seq_center.jpeg",
         dpi = "retina", width = 6, height = 6, units = "in")

vsnmPca_ExpStrategy <- pcaPlotting(pcaObject = prcomp(snmDataOGUFungi),
                       pcChoices = c(1,2),
                       dataLabels = qcMetadata$experimental_strategy,
                       factorString = "Sequencing Center",
                       titleString = "PCA w/ Voom-SNM Correction") +
  ggsave(filename = "Figures/Supplementary_Figures/tcga_pca_vsnm_exp_strategy.jpeg",
         dpi = "retina", width = 6, height = 6, units = "in")
...
# "Supplementary_scripts/S01-Run-pvca-fungi.R"
# "Supplementary_scripts/S01-pvca-function.R"
