#-----------------------------------------------------------------------------
# S19-ML-fungi-10k-rep1-tcga-stage.R
...
## Import data
load("data_for_ml_stage_29Oct21.RData")
...
datasetList <- list(rep200FungiDecontamV2SpeciesVSNM_Path_PT,
     rep200FungiDecontamV2SpeciesVSNM_CT_Path_PT)

datasetListNames <- c("rep200FungiDecontamV2SpeciesVSNM_Path_PT",
     "rep200FungiDecontamV2SpeciesVSNM_CT_Path_PT")

metadataList <- list(metaQiitaCombined_Nonzero_DecontamV2_Path_PT,
  metaQiitaCombined_Nonzero_DecontamV2_Path_PT)

metadataListNames <- list("metaQiitaCombined_Nonzero_DecontamV2_Path_PT",
  "metaQiitaCombined_Nonzero_DecontamV2_Path_PT")

baseNamePerDatasetResultsFile <- "perfFungi_10k_rep1_tcga_stage_DecontamV2_29Oct21"
baseNameAllResultsFile <- "perfFungi_10k_rep1_tcga_stage_ALL_DecontamV2_29Oct21"
  ...
  for(kk in seq_along(sampleTypeList)){
    ...
    if(st == "Primary Tumor vs Solid Tissue Normal"){
      ...
      metaTmp2 <- droplevels(metaTmp[(metaTmp$sample_type %in% "Primary Tumor") &
                                       (metaTmp$pathologic_stage_label_binned %in% c("StageI","StageIV")),])
    }
    ...
    for(ii in seq_along(levels(metaTmp2$disease_type))){
      ...
      if(st == "Primary Tumor vs Solid Tissue Normal"){
        ...
        positiveClass <- "StageIV"
        negativeClass <- "StageI"
      }
