#-----------------------------------------------------------------------------
# S18B-ML-fungi-vs-bacteria-number-reads.R
...
## Import data
load("data_for_ml_fungi_vs_bacteria_numFeat_with_AllSeqCenter_index_03Nov21.RData")
...
fungi_vs_bacteria_numReads_ML <- function(metaData = metaFungiVsBacteria,
                                          ...
                                          dbFlag = TRUE,
                                          dbPath = "checkpoints/rarefied_data_numReads_numIter10_PrimaryTumor_AllSeqCenters.RData",
                                          # seqCenter = "Harvard Medical School",
                                          taxTableFungi = rep200TaxSplit_Fungi,
                                          taxTableBacteria = rep200TaxSplit_Bacteria,
                                          modelType = "xgbTree",
                                          # readDepthVec = c(50, 500, 1000, 5000, 10e3, 15e3, 20e3, 25e3, 30e3, 35e3),
                                          readDepthVec = c(10e3, 15e3, 20e3, 25e3, 30e3, 35e3),
                                          numIter = 10,
                                          numResampleIter = 1,
                                          numKFold = 4){
  ...
  if(dbFlag){
    print("Loading rarefied data from RData file...")
    load(dbPath)
    } else{
      ...
      save(fungiDataListAll, 
       ...
       file = paste0("checkpoints/rarefied_data_numReads_numIter",numIter,"_",st,"_",sc,".RData"))
    }  
  ...
  for(jj in 1:length(readDepthVec)){ # should be length of readDepthVec
    ...
    for(ii in 1:numIter){ # should be numIter
      ...
      perfCombined[[ii]] <- cbind(fungiPerfDf, bacteriaPerfDf, 
                                  ...
                                  cancerTypes = paste(names(table(mlDataY$predY)), collapse = "|"))
      if((iterNum %% 5) == 0){
        checkpointPerfCombined <- perfCombined
        save(checkpointPerfCombined, file = paste0("checkpoints/checkpoint_numReads",readDepth,"_iterNum",iterNum,"_",st,"_",sc,"_k",numKFold,"_modelType_",modelType,".RData"))
      }
...
# Primary Tumor
fungi_vs_bacteria_numReads_ML(sampleType = "Primary Tumor")
# Blood Derived Normal
fungi_vs_bacteria_numReads_ML(sampleType = "Blood Derived Normal")
