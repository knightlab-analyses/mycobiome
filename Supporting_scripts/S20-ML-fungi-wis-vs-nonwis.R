...
# Purpose: Compare machine learning performance on WIS-intersected vs non-WIS-intersected fungi
#-----------------------------------------------------------------------------

#-------------------------------#
# Load dependencies
require(splitstackshape) # for stratified splits
require(tidyr)
...
load("indices_WIS_and_nonWIS_fungi_for_simulations.RData")
load("data_for_ml_fungi_vs_bacteria_numFeat_with_AllSeqCenter_index_29Oct21.RData")

# Primary Tumor
tmp_PT <- fungi_WIS_vs_nonWIS_ML(sampleType = "Primary Tumor", numIter = 5)
# Blood Derived Normal
tmpRF_BDN <- fungi_WIS_vs_nonWIS_ML(sampleType = "Blood Derived Normal", modelType = "rf")
