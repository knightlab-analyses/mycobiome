#-----------------------------------------------------------------------------
# S04-ML-fungi-10k-rep1-higher-taxa-tcga.R
...
## Import data
load("data_for_ml_tcga_wgs_vs_rna_13Sep21.RData")
...
sampleTypeList <- c("Primary Tumor vs Solid Tissue Normal",
                    # "Blood Derived Normal", # RNA data does not have any blood samples
                    "Primary Tumor")
# MODIFY AS NEEDED
datasetList <- list(snmDataOGUFungi_RNA)

# MODIFY AS NEEDED
datasetListNames <- c("snmDataOGUFungi_RNA")

# MATCHED METADATA DATA FRAME FOR ALL COUNT DATA:
metaTmpQC <- metaQiitaCombined_Nonzero_RNA # MODIFY AS NEEDED
metaTmpPath <- metaQiitaCombined_Nonzero_RNA # MODIFY AS NEEDED / PATHOLOGY METADATA DATA FRAME
...
for(jj in seq_along(datasetList)){
  ...
  write.csv(perfTmp2[[jj]], file = paste0("perfFungi_10k_rep1_tcga_rna_13Sep21_",datasetName,".csv"))
  write.csv(rep_perfTmp2[[jj]], file = paste0("rep_perfFungi_10k_rep1_tcga_rna_13Sep21_",datasetName,".csv"))
}
...
write.csv(perf1VsAll, file = paste0("perfFungi_10k_rep1_tcga_rna_ALL_13Sep21.csv"))
write.csv(rep_perf1VsAll, file = paste0("rep_perfFungi_10k_rep1_tcga_rna_ALL_13Sep21.csv"))
