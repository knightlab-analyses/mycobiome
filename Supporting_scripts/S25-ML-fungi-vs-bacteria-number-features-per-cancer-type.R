#-----------------------------------------------------------------------------
# S17B-ML-fungi-vs-bacteria-number-features.R
# Copyright (c) 2021--, Greg Poore
# Purpose: Run machine learning on all available TCGA cancer types varying number of taxa
#-----------------------------------------------------------------------------
...
fungi_vs_bacteria_numFeat_perCT_ML = function(metaData = metaFungiVsBacteria,
    ...
    for(kk in 1:length(dzVec)){
      ...
      for(ii in 1:numIter){ # should be 1:numIter
        ...
        if((iterNum %% 50) == 0){
          ...
          save(checkpointPerfCombined, file = paste0("checkpoints/checkpoint_perCT_numFeat",numFeatTmp,"_iterNum",iterNum,"_",st,"_",sc,"_k",numKFold,"_modelType_",modelType,".RData"))
        }
      }
  ...
  baseFilename <- paste0("fungi_vs_bacteria_perCT_numFeat_",paste(numFeatVec, collapse="_"),"_",st,"_",sc,"_numIter",numIter,"_k",numKFold,"_modelType_",modelType)
...
# Primary Tumor
fungi_vs_bacteria_numFeat_perCT_ML(sampleType = "Primary Tumor")
# Blood Derived Normal
fungi_vs_bacteria_numFeat_perCT_ML(sampleType = "Blood Derived Normal")
